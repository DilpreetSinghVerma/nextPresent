...
const RELAY_HOST = 'nextpresent-relay.onrender.com';
...
// ─── HTTP helper ──────────────────────────────────────────────────────────────
function getHttps(host, path, cookieHeader) {
 ...
 * Uses local cookie session (set by Electron's default session after OAuth).
 *
 * Returns: { isPro, email, name, avatar, plan, subscriptionExpiresAt }
 */
async function checkProStatus() {
  // 1. Try relay (user must be signed in via browser session)
  try {
    const result = await getHttps(RELAY_HOST, RELAY_PATH);
